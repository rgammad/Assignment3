...
void PLYObject::draw()
{
	...
	else
	{
		...
		Vector4f global_ambient = { 0.0,0.0,0.0,0.0 };
		Vector4f light_position = { 0.0,0.0,0.0,0.0 };

		GLfloat emission[4] = { 0.0,0.0,0.0,0.0 };
		Vector4f la, ld, ls;
		GLfloat d, kc, kl, kq;
		...
		glGetFloatv(GL_EMISSION, emission);
		glGetLightfv(GL_LIGHT0, GL_LIGHT_MODEL_AMBIENT, global_ambient);

		glGetLightfv(GL_LIGHT0, GL_POSITION, light_position);
		glGetLightfv(GL_LIGHT0, GL_CONSTANT_ATTENUATION, &kc);
		glGetLightfv(GL_LIGHT0, GL_LINEAR_ATTENUATION, &kl);
		glGetLightfv(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, &kq);
		...
		for (int v = 0; v < nv; v++)
		{
			...
			//material emission + global ambient * material ambient
			Vector3f temp;
			multVectors(temp, global_ambient, ambient);
			add(temp, emission);

			//find d: light_pos - vertices[v]. Distance between light and vertex
			sub(L, light_position, vertices[v]);
			d = normalize(L);
			//find summation
			GLfloat tempSum = (1 / (kc + kl*d + kq*pow(d, 2)));

			//get Half-Vector H of L and V.
			GLfloat hMag; //magnitute of H
			add(H, L, vertices[v]);
			hMag = sqrt(dotProd(H, H));
			scale(H, 1 / hMag);
			normalize(H);

			//light ambient * material ambient
			multVectors(amb, la, ambient);
			//max(L.N, 0) * light diffuse * material diffuse
			multVectors(dif, ld, diffuse);
			GLfloat lnMax;
			GLfloat dotL = dotProd(L, normals[v]);// = (dotProd(L, normals[v]) > 0 ? dotProd(L, normals[v]) : 0);
			if (dotL > 0)
				lnMax = dotL;
			else
				lnMax = 0;
			scale(dif, lnMax);
			//max(H.N, 0) ^ shininess * light specular * material specular
			multVectors(spe, ls, specular);
			GLfloat hnMax;
			GLfloat dotH = dotProd(H, normals[v]);// = (dotProd(H, normals[v]) > 0 ? dotProd(H, normals[v]) : 0);
			if (dotH > 0)
				hnMax = dotH;
			else
				hnMax = 0;
			scale(spe, pow(hnMax, shininess[0]));

			//ambient + diffuse + specular
			Vector3f tempTotal;
			add(tempTotal, amb, dif);
			add(tempTotal, spe);

			//summation * [ambient * diffuse * specular]
			scale(tempTotal, tempSum);

			//get total
			add(total, temp, tempTotal);
